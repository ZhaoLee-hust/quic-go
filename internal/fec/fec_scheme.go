...
type BlockFECScheme interface {
	...
	CanRecoverPackets(fecGroup *FECBlock) bool
}
...
func (f *XORFECScheme) CanRecoverPackets(group *FECBlock) bool {
	return group.CurrentNumberOfPackets() == group.TotalNumberOfPackets-1 && len(group.RepairSymbols) == 1
}

func (f *XORFECScheme) RecoverPackets(group *FECBlock) ([][]byte, error) {
	if !f.CanRecoverPackets(group) {
