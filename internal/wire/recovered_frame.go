...
type RecoveredFrame struct {
	// A. RecoveredRange has 2 elements: first and last, each of them have type protocol.PacketNumber.
	// B. has to be ordered. The highest ACK range goes first, the lowest ACK range goes last.
	RecoveredRanges []RecoveredRange
}
...
func ParseRecoveredFrame(r *bytes.Reader, version protocol.VersionNumber) (*RecoveredFrame, error) {
	...
	for i := uint8(0); i < numAckBlocks; i++ {
		...
		utils.Infof("blocklength = %d", ackBlockLength)
		if length > 0 {
...
func (f *RecoveredFrame) Write(b *bytes.Buffer, version protocol.VersionNumber) error {
	...
	utils.Infof("WRITE RECOVERED FRAME:")
	for _, r := range f.RecoveredRanges {
		utils.Infof("FROM %d TO %d", r.First, r.Last)
	}
	...
	for i, ackRange := range f.RecoveredRanges {
		...
		if num == 1 {
			...
			for i := 0; i < int(num); i++ {
				...
				utils.Infof("write %+v, num = %+v, i = %+v", gapWritten, num, i)
	...
	if numRanges != numRangesWritten {
		utils.Infof("numRanges = %+v, numRangesWritten = %+v", numRanges, numRangesWritten)
