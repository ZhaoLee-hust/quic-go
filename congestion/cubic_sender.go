...
// basically, if isRetransmittable return true
...
// true:被确认的最大pn<当拥塞窗口减少时的最大(待发送)pn,被确认的最大pn不为0
...
func (c *cubicSender) OnRetransmissionTimeout(packetsRetransmitted bool) {
	...
	utils.Debugf("RTO !, SET CWIN FROM %d TO %d", protocol.ByteCount(c.congestionWindow)*protocol.DefaultTCPMSS, protocol.ByteCount(c.minCongestionWindow)*protocol.DefaultTCPMSS)
	c.congestionWindow = c.minCongestionWindow
}
