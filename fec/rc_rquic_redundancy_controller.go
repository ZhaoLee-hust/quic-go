...
func (r *rquicRedundancyController) PushParamerters(paras TransParams) {
	...
	utils.Debugf("Saved: SntPkts %d,SntLost %d,SntRetrans %d,RcvPkts %d,Recover %d",
		r.PresentParas.SntPkts,
		r.PresentParas.SntLost,
		r.PresentParas.SntRetrans,
		r.PresentParas.RcvPkts,
		r.PresentParas.RecoveredPkts)
	utils.Debugf("Gamma:%f, Epsilon:%f, Epsilon[0]:%f, Epsilon[1]:%f, Epsilon[2]:%f", r.gamma, ArithmeticAverage(r.epsilon), r.epsilon[0], r.epsilon[1], r.epsilon[2])
	utils.Debugf("r.state: %d,r.epsilon[%d]: %f", r.state, r.state, r.epsilon[r.state])
	...
	utils.Debugf("CALLBACK: OnPacketLost:%d, OnPacketReceived:%d", r.packetLost, r.packetRec)
	utils.Debugf("------------------------------------------")
}
