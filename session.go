...
type session struct {
	connectionID protocol.ConnectionID  //连接ID
	perspective  protocol.Perspective   //视角
	version      protocol.VersionNumber //版本
	config       *Config                //参数

	paths     map[protocol.PathID]*path //路径
	pathsLock sync.RWMutex              //路径锁
	maxPathID protocol.PathID           //路径ID

	streamsMap   *streamsMap //流图
	cryptoStream streamI     //流

	rttStats *congestion.RTTStats //rtt

	remoteRTTs         map[protocol.PathID]time.Duration //路径rtt
	...
	unpacker unpacker      //解包器
	packer   *packetPacker //打包器

	peerBlocked bool //对端阻塞？

	cryptoSetup handshake.CryptoSetup

	receivedPackets  chan *receivedPacket //addr header data rcvtime rcvconn recovered
	recoveredPackets chan *receivedPacket //addr header data rcvtime rcvconn recovered
...
var newClientSession = func(
	...
	utils.Infof("client FEC SCHEME: %v", fs)
...
func (s *session) setup(
	...
	if s.config.RedundancyController == nil {
		// s.redundancyController = fec.NewConstantRedundancyController(
		// 	uint(protocol.NumberOfFecPackets),
		// 	uint(protocol.NumberOfRepairSymbols),
		// 	uint(protocol.NumberOfInterleavedFECGroups),
		// 	uint(protocol.ConvolutionalStepSize))

		// s.redundancyController = fec.NewAverageRedundancyController(
		// 	uint8(protocol.NumberOfFecPackets),
		// 	protocol.NumberOfRepairSymbols)

		// default redundancyController
		utils.Infof("No redundancyController, Set NewrQuicRedundancyController as default")

		s.redundancyController = fec.NewrQuicRedundancyController(
			uint8(protocol.NumberOfFecPackets),
			uint8(protocol.NumberOfRepairSymbols))
	...
	s.fecFrameworkSender = NewFECFrameworkSender(s.senderFECScheme, s.fecScheduler, s.fecFramer, s.redundancyController, s.version, s)
	s.bulkRecovery = true

	// s.paths[protocol.InitialPathID].sentPacketHandler
...
runLoop:
	for {
		...
		case <-s.sendingScheduled:
			// We do all the interesting stuff after the switch statement, so
			// nothing to see here.
		case tmpPth := <-s.pathTimers: //在path的runloop中
			timerPth = tmpPth
			// We do all the interesting stuff after the switch statement, so
			// nothing to see here.
...
// 当session收到packet时会调用，对数据包进行解密，然后将packet的frames进行处理
func (s *session) handlePacketImpl(p *receivedPacket) error {
	...
	// zhaolee: use InitialPath handle recovered packets?
	if p.recovered {
...
func (s *session) handleRecoveredFrame(frame *wire.RecoveredFrame, pid protocol.PathID, encLevel protocol.EncryptionLevel) error {
	pth := s.paths[pid]
	utils.Infof("HANDLE RECOVERED FRAME:")
	for _, r := range frame.RecoveredRanges {
		utils.Infof("FROM %+v TO %+v", r.First, r.Last)
	}
...
// 发送方真正实施发送，先调用拥塞控制，再pth.conn.Write(packet.raw)
func (s *session) sendPackedPacket(packet *packedPacket, pth *path) error {
	...
	// add by zhaolee
	var foundFinbitInStreamFrame bool
